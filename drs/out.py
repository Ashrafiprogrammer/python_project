...
# def play(Speed):
#     global flag
#     print("Your clicked on play.speed is {Speed}")
#     frame1 = stream.get(cv2.CAP_PROP_FRAME_COUNT)
...
#                            font="Times 26 bold", text='Decission pending')
#     flag = not flag


# def pending(decision):
#     frame = cv2.cvtColor(cv2.imread("pending.png"), cv2.COLOR_BGR2RGB)
#     frame = imutils.resize(frame, width=SET_WIDTH, height=HEIGHT)
#     frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
#     canvas.image = frame
#     canvas.create_image(0, 0, image=frame, anchor=tkinter.NW)
#     time.sleep(2)
...
# def out():
#     thread = threading.Thread(target=pending, arge=('out'))
#     thread.daemon = 1
#     thread.start()
#     print("player is out")
